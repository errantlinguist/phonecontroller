...
class PhoneControllerMainFrame(private val phoneControllerManager: PhoneControllerManager, title0: String, private val settingsFrame: Frame) extends MainFrame {
	...
	import com.github.errantlinguist.snom.PhoneKeyEvent
	...
	private val createHelpMenu: (String) ⇒ Menu = title ⇒ {
		new Menu(title) {
			...
			contents += new MenuItem(Action("About...") {
				try {
					val aboutMessage = PhoneControllerMainFrame.AboutMessageFactory.create()
					Dialog.showMessage(this, aboutMessage, "About", Dialog.Message.Info)
				} catch {
					case e: Exception => {
						val errorClassName = e.getClass().getSimpleName()
						val errorMessage = "An %s occurred while loading the application description file \"%s\".%n%nMessage: %s".format(errorClassName, PhoneControllerMainFrame.AboutMessageResourceName, e.getMessage())
						Dialog.showMessage(this, errorMessage, errorClassName, Dialog.Message.Error)
					}
				}
			})
		}
...
private object PhoneControllerMainFrame {
	private val AboutMessageResourceName = "app.properties"
	private val AboutMessageFactory = new ApplicationInfoMessageFactory(AboutMessageResourceName)
}
