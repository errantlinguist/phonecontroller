...
class PhoneControllerPanel(var phoneControllerManager: PhoneControllerManager) extends BorderPanel {
	...
	private def showErrorMessage(t: Throwable) {
		t.printStackTrace()
		val throwableClassName = t.getClass().getSimpleName()
		Dialog.showMessage(this, "An unexpected %s occurred.%nMessage: %s%n%nCheck your settings.".format(throwableClassName, t.getMessage()), throwableClassName, Dialog.Message.Error)
	}
	
	private def showTimeoutErrorMessage() {
		Dialog.showMessage(this, "The network connection to the phone timed out; Check your settings.", "Network timeout", Dialog.Message.Error)
	}
				...
				case b: ButtonClicked if b == dialButton ⇒ {
					...
					if (input != null && input.matches(DialNumberPattern)) {
						try {
							...
							case e: SocketTimeoutException ⇒ showTimeoutErrorMessage
							case e: IOException ⇒ showErrorMessage(e)
						}
